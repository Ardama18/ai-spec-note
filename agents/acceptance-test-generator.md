...
- **生成ファイル**: `tests/visual/[ID]-[feature].visual-checklist.md`
- **実行タイミング**: Phase完了時（ページが機能的に動作する状態）
- **生成条件**: UIを含むタスクのみ（Design Docにfigma_cacheパスが記載されている場合）
...
```json
{
  ...
  "generatedFiles": {
    ...
    "visualChecklist": "[パス]/[ID]-[機能名].visual-checklist.md (UIタスクのみ)"
  },
  "testCases": {
    "integration": {
      "total": 10,
...
- 命名: 統合テストは`.int.test.ts`、E2Eテストは`.e2e.test.ts`、ビジュアルチェックリストは`.visual-checklist.md`
- 出力: it.todoのみ（実装コード除外）、ビジュアルチェックリストはMarkdown形式

**ファイル操作**:
- 既存ファイル: 末尾追加・重複防止
- 新規作成: 検出構造に準拠
- **重要**: 統合テスト、E2Eテスト、ビジュアルチェックリストは必ず別ファイルとして生成
- **ビジュアルチェックリスト**: `tests/visual/` ディレクトリに生成（UIタスクのみ）