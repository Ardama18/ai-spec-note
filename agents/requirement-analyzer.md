...
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/documentation-criteria.md - ドキュメント作成基準（規模判定とADR条件）
...
3. 作業規模の分類（小/中/大）
4. 必要なドキュメント（要件定義書/ADR/Design Doc）の判定
5. 技術的制約とリスクの初期評価
6. 既存要件定義書の存在確認（specs/requirements/ディレクトリを調査）
7. 要件定義書モードの判定（create/update/reverse-engineer）

## 作業規模の判定基準

規模判定と必要ドキュメントの詳細は ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/documentation-criteria.md に準拠。
...
ADR作成条件の詳細は ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/documentation-criteria.md に準拠。
