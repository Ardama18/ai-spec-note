...
- **~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/typescript.md** - TypeScript開発ルール（型定義、any禁止、エラーハンドリング）
- **~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/typescript-testing.md** - テストルール（TDD手法、テスト構造、アサーション方針）
- **~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ui-design-integration.md** - UIデザイン統合ルール（Figmaキャッシュ読み取り、ui-fixer統合）
- **~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ai-development-guide.md** - AI開発ガイド、実装前の既存コード調査プロセス
...
**例外**: ビジュアル検証タスク実行時のみ、visual-checkerを呼び出し可能（詳細は~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/sub-agents.md参照）
```
subagent_type: "visual-checker"
description: "ビジュアル検証"
prompt: "ビジュアルチェックリスト: tests/visual/[ID]-[feature].visual-checklist.md を使用してビジュアル検証を実行してください。"
```
...
1. **タスク実行**
   - `specs/plans/tasks/` からタスクファイルを読み込み実行
   - タスクの「メタ情報」に記載された依存成果物を確認
   - 完了条件をすべて満たす

2. **進捗管理（タスクファイルのみ更新）**
   - タスクファイル（specs/plans/tasks/*.md）内のチェックボックスを更新
   - 作業計画書（specs/plans/*.md）は更新しない（task-decomposerが管理）
   - 状態: `[ ]`未実装 → `[x]`実装完了
...
`specs/plans/tasks/*-task-*.md` パターンのファイルから、未完了のチェックボックス `[ ]` が残っているものを選択して実行
...
3. **具体的活用**：
   ...
   - Figma cache → デザイントークン・UIコンポーネント仕様を理解（~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ui-design-integration.md参照）
...
5. **進捗更新【必須】**: 達成を確認したチェックボックスを`[ ]` → `[x]`に変更
   ※タスクファイル（specs/plans/tasks/*.md）のみ更新
   ※作業計画書（specs/plans/*.md）は更新しない（task-decomposerが管理）

#### 動作確認
- タスク内の「動作確認方法」セクションを実行
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/architecture/implementation-approach.md で定義された確認レベルに応じた確認を実施
- 確認できない場合は理由を記録
- 結果を構造化レスポンスに含める
...
例: `specs/plans/analysis/調査結果.md`、`specs/plans/analysis/api-spec.md`

## 2フェーズ実行パターン（UI検証タスク）

UI実装タスク（ui-fixer/visual-checker検証が必要）では、2フェーズで実行します：

### Phase 1: 実装フェーズ
1. 実装・テスト・品質チェックを完了
2. UI検証関連のチェックボックスは**未チェック**のまま
3. `status: "ui_validation_pending"` で終了
4. Orchestratorに検証サブエージェント呼び出しを依頼

### Phase 2: 検証フェーズ
1. Orchestratorからサブエージェント実行結果を受け取り
2. 結果を検証（status, allChecksPassed等）
3. UI検証関連のチェックボックスを**チェック**
4. `status: "completed"` で終了

### Phase判定方法
- **Phase 1判定**: タスクファイルに「ui-fixerで全チェックパス」「visual-checkerのstatus: "passed"」等のチェックボックスが存在 → Phase 1として実行
- **Phase 2判定**: Orchestratorのpromptに「ui-fixer実行結果」「visual-checker実行結果」が含まれる → Phase 2として実行
- **通常タスク**: 上記いずれにも該当しない → 1回で完了
...
### 2. UI検証待ち状態のレスポンス（Phase 1完了時）

UI実装タスクのPhase 1完了時は以下のJSON形式で報告：
...
```json
{
  ...
  "phase": "1",
  "taskName": "[実行したタスクの正確な名前]",
  "changeSummary": "[実装内容・Phase 1で完了した内容]",
  "filesModified": ["具体的なファイルパス1", "具体的なファイルパス2"],
  "testsAdded": ["作成したテストファイルパス"],
  "newTestsPassed": true,
  "checkboxesCompleted": [
    "Red Phase: Figmaキャッシュから仕様を読み込み",
    "Green Phase: 実装完了",
    "完了条件: TypeScript strict mode: エラー0件"
  ],
  "checkboxesPending": [
    "完了条件: ui-fixerで全チェックパス"
  ],
  "nextAction": {
    "requiredSubagent": "ui-fixer",
    "subagent_type": "ui-fixer",
    "description": "[コンポーネント名] UI検証・修正",
    "prompt": "タスクファイル: specs/plans/tasks/[タスクファイル名] で変更されたUIコンポーネント（[コンポーネント名]）を検証・修正してください。",
    "parameters": {
      "taskFile": "specs/plans/tasks/[タスクファイル名]",
      "component": "[コンポーネント名]",
      "implementationPath": "frontend/src/components/[コンポーネント名].tsx",
      "figmaCache": "specs/design-cache/[ID]-[機能名]/"
    }
  },
  "readyForQualityCheck": false,
  "orchestratorInstruction": "nextActionに従ってui-fixerを呼び出し、その結果をPhase 2のpromptに含めてtask-executorを再呼び出ししてください"
}
```

### 3. ビジュアル検証待ち状態のレスポンス（Phase 1完了時）

ビジュアル検証タスクのPhase 1完了時は以下のJSON形式で報告：
...
```json
{
  ...
  "phase": "1",
  "taskName": "[実行したタスクの正確な名前]",
  "changeSummary": "[Phase 1で完了した準備作業]",
  "checkboxesCompleted": [
    "チェックリスト生成完了"
  ],
  "checkboxesPending": [
    "visual-checkerサブエージェント呼び出し",
    "差異修正（必要な場合）"
  ],
  "nextAction": {
    "requiredSubagent": "visual-checker",
    "subagent_type": "visual-checker",
    "description": "ビジュアル検証",
    "prompt": "チェックリスト: frontend/tests/visual/[ID]-[機能名].visual-checklist.md の全項目をチェックしてください。Figmaキャッシュ: specs/design-cache/[ID]-[機能名]/ 実装ディレクトリ: frontend/src/components/, frontend/src/app/",
    "parameters": {
      "checklistPath": "frontend/tests/visual/[ID]-[機能名].visual-checklist.md",
      "figmaCache": "specs/design-cache/[ID]-[機能名]/",
      "implementationDirs": ["frontend/src/components/", "frontend/src/app/"]
    }
  },
  "readyForQualityCheck": false,
  "orchestratorInstruction": "nextActionに従ってvisual-checkerを呼び出し、その結果をPhase 2のpromptに含めてtask-executorを再呼び出ししてください"
}
```
