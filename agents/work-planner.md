...
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/typescript-testing.md - テストルール
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ui-design-integration.md - UIデザイン統合ルール（ui-fixer統合パラメータ仕様）
...
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/architecture/implementation-approach.md - 実装戦略パターンと確認レベル定義（タスク分解で使用）
...
### ID・機能名の引き継ぎ
1. **Design Docから取得**: 入力されたDesign Docのメタデータセクションからが読み取る
2. **同じIDを使用**: Design Docと同一のID・機能名を使用
3. **記録**: 作業計画書のメタデータセクションに明記

### 出力ファイル名
- **形式**: `specs/plans/[ID]-[機能名]-plan.md`
- **例**: `specs/plans/F001-user-auth-plan.md`
...
version: 1.0.0
created: 2025-01-15
based_on: specs/design/F001-user-auth-design.md
---
```
...
1. **Design DocからFigmaキャッシュパスを取得**
   ...
   - 例: `specs/design-cache/F001-user-profile/`
...
3. **各UIタスクの完了条件にui-fixerパラメータを明記**
   - ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ui-design-integration.md の「パラメータ仕様」に従って記載
...
version: 1.0.0
created: 2025-01-15
based_on: specs/design/F001-user-profile-design.md
figma_cache: specs/design-cache/F001-user-profile/
---
```
...
- [ ] 単体テスト作成・実行完了
- [ ] **ui-fixerで全チェックパス**
  - cache: specs/design-cache/F001-user-profile/
  - component: Button
...
- 保存場所: `specs/plans/[ID]-[機能名]-plan.md` (例: F001-user-auth-plan.md)
  - ID: 4桁（F001, F002...）
- 命名規則は ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/documentation-criteria.md に従って作成
- チェックボックス形式のタスクリスト
...
7. **テストファイルパスの明記**: 各テストタスクに具体的なテストファイルパスを記載（~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/typescript-testing.mdのディレクトリ構造に従う）
...
**テストファイルパスの明記（必須）**:
各テストタスクには、~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/typescript-testing.mdのディレクトリ構造に従って、具体的なテストファイルパスを明記すること。
...
### 実装アプローチの適用
Design Docで決定された実装アプローチと技術的依存関係に基づき、~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/architecture/implementation-approach.mdの確認レベル（L1/L2/L3）に従ってタスクを分解する。
