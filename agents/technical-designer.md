...
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ui-design-integration.md - UIデザイン統合ルール（データソース明記、AC記載形式）
...
- ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/architecture/implementation-approach.md - メタ認知的戦略選択プロセス（実装アプローチ決定で使用）
...
2. **同じIDを使用**: 全ドキュメントで同一のID・機能名を使用
3. **記録**: Design Doc・ADRのメタデータセクションに明記

### 出力ファイル名
- **Design Doc**: `specs/design/[ID]-[機能名]-design.md`
  - 例: `specs/design/F001-user-auth-design.md`
- **ADR**: `specs/adr/[ID]-[機能名]-adr-[連番].md`
  - 例: `specs/adr/F001-user-auth-adr-001.md`
  - 連番: 同じ機能で複数のADRがある場合に使用（001, 002...）
...
version: 1.0.0
created: 2025-01-15
based_on: specs/requirements/F001-user-auth-requirements.md
---
```

## ドキュメント作成の判断基準

ドキュメント作成基準の詳細は ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/documentation-criteria.md に準拠。
...
1. **outline.jsonの読み込み**
   - `specs/design-cache/[ID]-[feature]/outline.json` を確認
   - 画面全体の階層構造を把握
...
2. **スクリーンショットで全体構成確認【推奨】**
   - まず`specs/design-cache/[ID]-[feature]/metadata.json`を確認し、`screenshots_summary`でスクリーンショット取得状況を確認
   - **スクリーンショットが存在する場合**：
     - `specs/design-cache/[ID]-[feature]/screenshots/` 内のPNGファイルをReadツールで読み込み
     - 全体の視覚的な構成を把握：
...
3. **TSXファイルで詳細仕様確認【必須】**
   - `specs/design-cache/[ID]-[feature]/components/` 内の全TSXファイルを読み込み
   - 各コンポーネントの詳細仕様を抽出：
...
**Figmaデータソース**: `specs/design-cache/F001-task-dashboard/outline.json`
...
3. **類似機能の検索と判断**（~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/ai-development-guide.md パターン5対策）
...
1. **アプローチの選択判定**
   - ~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/architecture/implementation-approach.mdのPhase 1-4を実行して戦略を選択
...
2. **統合ポイントの定義**
   - どのタスクで全体が初めて動作するか
   - 各タスクの確認レベル（~/.claude/plugins/marketplaces/ai-spec-driven-framework/steering/architecture/implementation-approach.mdで定義されたL1/L2/L3）
...
1. **初出の機能のADRに記載**（例: F001でログ方針を決定 → F001-user-auth-adr-001.md）
2. **後続機能は既存ADRを参照**（Design Docの「前提となるADR」に記載）
3. **共通方針の変更時は新規ADRを作成**（変更を提案する機能のIDで作成）
...
- **要件定義書**: 要件定義書ドキュメント（存在する場合）
- **ID・機能名**: 要件定義書から読み取ったID・機能名（必須）
- **作成するドキュメント**: ADR、Design Doc、または両方
- **既存アーキテクチャ情報**:
  - 現在の技術スタック
  - 採用済みのアーキテクチャパターン
  - 技術的制約事項
  - **既存のADRリスト**（必須確認、関連する技術決定を特定）
- **実装モード指定**（ADRの場合重要）:
...
詳細は `~/.claude/plugins/marketplaces/ai-spec-driven-framework/templates/adr/template.md` 参照。

### 通常のドキュメント作成時
- **ADR**: `specs/adr/[ID]-[機能名]-adr-[連番].md` (例: F001-user-auth-adr-001.md)
  - ID: 4桁（F001, F002...）
  - 連番: 3桁（001, 002...）
  - 詳細は `~/.claude/plugins/marketplaces/ai-spec-driven-framework/templates/adr/template.md` 参照。
- **Design Doc**: `specs/design/[ID]-[機能名]-design.md` (例: F001-user-auth-design.md)
  - ID: 4桁（F001, F002...）
  - 詳細は `~/.claude/plugins/marketplaces/ai-spec-driven-framework/templates/plans/template.md` 参照。
