...
version: 1.0.0
created: [YYYY-MM-DD]
based_on: specs/design/[ID]-[機能名]-design.md
figma_cache: [specs/design-cache/[ID]-[機能名]/]（UI実装を含む場合のみ。不在の場合は "none"）
---
...
## 関連ドキュメント
- ADR: [specs/adr/[ID]-[機能名]-adr-[連番].md]（あれば）
- 要件定義書: [specs/requirements/[ID]-[機能名]-requirements.md]（あれば）
- Figmaキャッシュ: [specs/design-cache/[ID]-[機能名]/]（UI実装を含む場合のみ）
  - outline.json: 画面構造
  - screenshots/: 視覚確認用
  - components/: 詳細仕様（TSX）
...
- [ ] Button コンポーネント実装
  ...
  - cache: specs/design-cache/[ID]-[機能名]/
  - component: Button
